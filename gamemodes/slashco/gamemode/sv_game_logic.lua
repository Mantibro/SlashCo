...
local function permute(lim)
	...
	math.random() -- extra random to try and make generators less not-random
	local shift = n // 2
...
SlashCo.ValidateMap = function(map)
	...
	SlashCo.CurRound.HelicopterSpawnPosition = Vector(json.Helicopter.StartLocation.pos[1],
			json.Helicopter.StartLocation.pos[2], json.Helicopter.StartLocation.pos[3])

	SlashCo.CurRound.HelicopterIntroPosition = Vector(json.Helicopter.IntroLocation.pos[1],
			json.Helicopter.IntroLocation.pos[2], json.Helicopter.IntroLocation.pos[3])

	SlashCo.CurRound.HelicopterIntroAngle = Angle(json.Helicopter.IntroLocation.ang[1],
			json.Helicopter.IntroLocation.ang[2], json.Helicopter.IntroLocation.ang[3])
	...
	if gasCount > #(json.GasCans.Spawnpoints) then
		MsgC(Color(255, 50, 50),
				"[SlashCo] This map has too few gas can spawnpoints for the number of gas cans required! (" .. tostring(gasCount) .. " Gas Cans > " .. tostring(#(json.GasCans.Spawnpoints)) .. " Gas Can Spawnpoints), this map will not be playable!\n")
		valid = false
	end

	if gasCount < genCount * SlashCo.GasCansPerGenerator then
		MsgC(Color(255, 50, 50),
				"[SlashCo] This map has too few gas cans for the number of given generators (" .. tostring(gasCount) .. " Gas Cans < " .. tostring(genCount) .. " Generators*4), this map will not be playable!\n")
		valid = false
	end

	if batCount < genCount then
		MsgC(Color(255, 50, 50),
				"[SlashCo] This map has too few battery spawnpoints for the number of given generators (" .. tostring(batCount) .. " Gas Cans < " .. tostring(genCount) .. " Generators), this map will not be playable!\n")
	end
	...
	for I = 1, #(json.Batteries.Spawnpoints) do
		if #(json.Batteries.Spawnpoints[I]) == 0 then
			MsgC(Color(255, 50, 50),
					"[SlashCo] There are no battery spawnpoints available for Generator spawnpoint " .. tostring(I) .. "! This could cause issues if a generator spawns here, as it will have no battery, making the map non-completable! Exiting for safety, fix your JSON.\n")
			valid = false
		end
	end
	...
	if SlashCo.CurRound.OfferingData.CurrentOffering == 1 then
		--The Exposure Offering caps gas cans at 8.
	...
	if json.Offerings.Exposure.Spawnpoints then
		if #(json.Offerings.Exposure.Spawnpoints) == 0 then
			MsgC(Color(255, 50, 50),
					"[SlashCo] This configuration has no spawnpoints for the Exposure offering. It will not work.\n")
		elseif #(json.Offerings.Exposure.Spawnpoints) < SlashCo.CurRound.GasCanCount then
			MsgC(Color(255, 50, 50),
					"[SlashCo] This configuration doesn't have enough spawnpoints for the Exposure offering. It will not work.\n")
		end
	else
		MsgC(Color(255, 50, 50),
				"[SlashCo] This configuration has no data for the Exposure offering. It will not work.\n")
	end
	...
	if curmap ~= "sc_lobby" then
		...
		if item_count >= 1 then
			...
			if #random_itemSpawns >= 1 then
				for _ = 1, #random_itemSpawns do
					--Free items spawned during the round
		...
		for _, p in ipairs(SlashCo.CurRound.SlashersToBeSpawned) do
			...
			if itemClass then
				SlashCo.CreateItems(itemSpawns, itemClass)
				print("[SlashCo] Spawning Items.")
			end
		...
		local id = SlashCo.CreateItem("sc_beacon", Vector(pickedpoint.pos[1], pickedpoint.pos[2], pickedpoint.pos[3]),
				Angle(pickedpoint.ang[1], pickedpoint.ang[2], pickedpoint.ang[3])) --Spawn one distress beacon
...
SlashCo.TestConfig = function()
	...
	local genSpawns = SlashCo.GetSpawnpoints(#(SlashCo.CurConfig.Generators.Spawnpoints),
			#(SlashCo.CurConfig.Generators.Spawnpoints))
	local gasSpawns = SlashCo.GetSpawnpoints(#(SlashCo.CurConfig.GasCans.Spawnpoints),
			#(SlashCo.CurConfig.GasCans.Spawnpoints))
	...
	if SlashCo.CurConfig.Offerings.Exposure.Spawnpoints then
		local exposureSpawns = SlashCo.GetSpawnpoints(#(SlashCo.CurConfig.Offerings.Exposure.Spawnpoints),
				#(SlashCo.CurConfig.Offerings.Exposure.Spawnpoints))

...
hook.Add("HUDPaint", "OfferingVoteHUD", function()
	...
	if show_offering_result_screen == true then
		...
		draw.SimpleText( SlashCoLanguage("offervote_success", GetOfferingName(offering_name)), "LobbyFont2", ScrW() * 0.5, (ScrH() * 0.5), Color( 255, 255, 255, math.abs(o_tick) ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
	...
	draw.SimpleText( SlashCoLanguage("offervote_1", offeror_name, GetOfferingName(offering_name)), "LobbyFont1", ScrW() * 0.5, (ScrH() * 0.27), color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )

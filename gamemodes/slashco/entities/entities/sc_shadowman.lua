...
function ENT:Initialize()
	...
	if LocalPlayer().BenadrylIntensity then
		self:EmitSound("slashco/benadryl_shadow"..self.Sound..".mp3",60+math.random(1,80), 100, (math.random() * 2) * LocalPlayer().BenadrylIntensity)
	end
...
function ENT:Think()
	...
	if not LocalPlayer().BenadrylIntensity then return end
	...
	if not self.Cycle then self.Cycle = CurTime() end
